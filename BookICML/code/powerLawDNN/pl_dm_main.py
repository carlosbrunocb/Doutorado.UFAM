...
from model.pl_analysis import *
from model.pl_apl_model import *
from depth_map.pl_depth_maps import *
from model.pl_loss_function import *
from model.pl_metric_function import *
...
def main():
    ...
    drive_path = 'dataset'
    # mask_path = 'dataset/hazy_depth_maps/512x512/v1'
    mask_path = 'dataset/apl/dm/v2'
    # sp_dir = 'hazy'
    sp_dir = 'apl/res/512x512'  # APL results [global]
    gt_dir = 'hazy_gt'
    ...
    sp_datase, gt_dataset = load_images_from_folder(drive_path,
                                                    ...
                                                    resize=True)
    dm_batch = load_batch_images_from_folder(mask_path,
                                             width=dim[0], height=dim[1],
                                             resize=True, normalized=True,
                                             grayscale=True)
    mask_batch = generate_depth_masks_from_intervals(dm_batch, num_intervals)

    print(f"num_intervals = {num_intervals}")
    print(f"sp_datase = {sp_datase.shape}")
    print(f"gt_dataset = {gt_dataset.shape}")
    print(f"dm_batch = {dm_batch.shape}")
    print(f"mask_batch = {mask_batch.shape}")

    # Print inputs
    # for i, mask_list in enumerate(mask_batch):
    ...
    #         plt.subplot(1, num_intervals, j + 1)
    ...
    train_x, train_m, train_y = img_train[train_ix], img_mask[train_ix], img_gt[train_ix]
    test_x, test_m, test_y = img_train[test_ix], img_mask[test_ix], img_gt[test_ix]
    ...
    dummy_train_params = np.zeros((len(train_y), 3 * num_intervals), dtype=np.float32)
    dummy_test_params = np.zeros((len(test_y), 3 * num_intervals), dtype=np.float32)

    print(f"train_x = {train_x.shape}")
    print(f"train_x = {train_m.shape}")
    print(f"train_y = {train_y.shape}")
    print(f"dummy_train_params = {dummy_train_params.shape}")
    print(f"test_x = {test_x.shape}")
    print(f"test_x = {test_m.shape}")
    print(f"test_y = {test_y.shape}")
    print(f"dummy_test_params = {dummy_test_params.shape}")

    # dir_name = 'results/dehaze_apl/512x512'
    ...
    history = model.fit([train_x, train_m],  # Inputs: image and mask
                        [train_y, dummy_train_params],  # Outputs: transformed image and dummy parameters
                        epochs=n_epochs,
                        batch_size=n_batch,
                        shuffle=True,
                        validation_data=([test_x, test_m], [test_y, dummy_test_params]),
    ...
    results = model.evaluate([test_x, test_m], test_y, verbose=1)
    ...
    # predicted_img, predictions_params = model.predict([test_x, test_m])
    predicted_img, predictions_params = model.predict([img_train, img_mask])
